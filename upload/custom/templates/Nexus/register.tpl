...
<div class="ui padded segment" id="register" style="border-radius:0px;">
				...
				<form class="ui form" action="" method="post" id="form-register">
					{if isset($NICKNAMES)}
						<div class="field">
							...
							<input type="text" name="{if isset($MINECRAFT)}nickname{else}username{/if}" id="username"
								value="{if isset($MINECRAFT)}{$NICKNAME_VALUE}{else}{$USERNAME_VALUE}{/if}" placeholder="{$NICKNAME}"
								tabindex="1">
						</div>
						{if isset($MINECRAFT)}
							<div class="field">
								<label>{$MINECRAFT_USERNAME}</label>
								<input type="text" name="username" id="mcname" value="{$USERNAME_VALUE}" placeholder="{$MINECRAFT_USERNAME}"
									tabindex="2">
							</div>
						{/if}
					{else}
						...
						{else}
							<div class="field">
								...
								<input type="text" name="username" id="mcname" value="{$USERNAME_VALUE}" placeholder="{$NICKNAME}"
									tabindex="1">
							</div>
						{/if}
					{/if}
					...
					<div class="field">
						...
						<input type="password" name="password_again" id="password_again" placeholder="{$CONFIRM_PASSWORD}"
							tabindex="5">
					</div>
						...
						{foreach $CUSTOM_FIELDS as $field}
							<div class="field">
								...
								{elseif $field.type eq 2}
									<textarea name="{$field.name}" id="{$field.name}" placeholder="{$field.description}"
										tabindex="5"></textarea>
					...
					<input type="hidden" name="token" value="{$TOKEN}">
 	                <input id="timezone" type="hidden" name="timezone" value=''>
						...
						<input type="submit" class="ui primary button" style="width: 100%;" value="{$REGISTER}" tabindex="8">
