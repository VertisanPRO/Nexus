...
<div class="ui stackable three column grid" {if isset($BANNER_IMAGE)}
		...
		<div class="ui steps">
			<div class="step" style="background: transparent;color: white;">
				...
				<div class="content">
					...
					<div class="description" style="color: white;">{$DISCORD_SERVER['members']} members online</div>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="column" onclick="copyIP('{$OPAL_MCSERVER_IP}:{$OPAL_MCSERVER_PORT}')" data-tooltip="{$CLICK_TO_COPY_TOOLTIP}" data-variation="mini" data-inverted="" data-position="bottom center
...
<div class="ui secondary {$DEFAULT_REVAMP_NAVBAR_EXTRA_CLASSES} {if isset($NAVBAR_SIZE)}{$NAVBAR_SIZE}{else}small{/if} menu" id="navbar"
	style="background: {if isset($TEMPLATE_COLOR)} {$TEMPLATE_COLOR}{else} #7e31e3{/if}; margin-bottom:20px;">
