<div class="ui fluid card" id="widget-latest-posts">
  <div class="content">
    <a class="ui ribbon label" style="margin-bottom: 10px;"><i class="fa-solid fa-comment"></i>
      {$LATEST_POSTS}</a>
    <div class="description">
      {foreach from=$LATEST_POSTS_ARRAY name=latest_posts item=post}
      <div class="ui relaxed list">
        <div class="item">
          <img class="ui mini circular image" src="{$post.last_reply_avatar}" alt="{$post.last_reply_username}">
          <div class="content">
            ...
            <a href="{$post.last_reply_profile_link}" style="{$post.last_reply_style}"
              data-poload="{$USER_INFO_URL}{$post.last_reply_user_id}">{$post.last_reply_username}</a> &middot; <span
              data-toggle="tooltip" data-content="{$post.last_reply}">{$post.last_reply_rough}</span>
          </div>
        </div>
      </div>
      {/foreach}
