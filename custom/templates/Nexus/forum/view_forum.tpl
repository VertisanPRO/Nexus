    ...
    <div class="ui {if count($WIDGETS_LEFT) && count($WIDGETS_RIGHT) }four wide tablet eight wide computer{elseif count($WIDGETS_LEFT) || count($WIDGETS_RIGHT)}ten wide tablet twelve wide computer{else}sixteen wide{/if} column">
      {if count($SUBFORUMS)}
        <table class="ui fixed single line selectable unstackable small padded res table" id="subforums-table">
          <thead>
            <tr>
              <th class="nine wide"><h4>{$SUBFORUM_LANGUAGE}</h4></th>
      ...
      {if count($STICKY_DISCUSSIONS)}
        <table class="ui fixed single line selectable unstackable small padded res table" id="sticky-threads">
          <thead>
            <tr>
              <th class="eight wide"><h4>{$STICKY_TOPICS}</h4></th>
                  ...
                  <h5 class="ui header">
                    ...
                    <div class="content">
                      ...
                      <div class="ui wide popup">
                        ...
                        {$BY|capitalize} <a href="{$discussion.author_link}" style="{$discussion.topic_created_style}">{$discussion.topic_created_username}</a> | {$discussion.topic_created}
                      </div>
                      <div class="sub header">
                        <a href="{$discussion.author_link}" data-poload="{$USER_INFO_URL}{$discussion.topic_created_user_id}" style="{$discussion.topic_created_style}">{$discussion.topic_created_username}</a> &middot; <span data-toggle="tooltip" data-content="{$discussion.topic_created}">{$discussion.topic_created_rough}</span>
                      </div>
                   </div>
                  </h5> 
                </td>
      ...
      {if count($LATEST_DISCUSSIONS)}
        <table class="ui fixed single line selectable unstackable small padded res table" id="normal-threads">
          <thead>
            <tr>
              <th class="eight wide"><h4>{$TOPICS|capitalize}</h4></th>
              <th class="three wide"></th>
              <th class="five wide"></th>
            </tr>
          </thead>
