...
<html{$HTMLCLASS}{$HTMLLANG}{$HTMLRTL}>
  <head>
    ...
    <link rel="shortcut icon" href="{if isset($FAVICON)}{$FAVICON}{else}https://cdn.resourcemc.net/zAsa7/rIBOyeRU58.png/raw{/if}">
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
      ...
      <title>{$TITLE} • {$smarty.const.SITE_NAME}</title>
		...
		<meta name="description" content="{if isset($OPAL_META_HEADING)}{$OPAL_META_DESCRIPTION}{else}We are an online platform to help users around the world connect through NamelessMC powered forums.{/if}">
		<meta name="keywords" content="{if isset($OPAL_META_KEYWORDS)}{$OPAL_META_KEYWORDS}{else}Minecraft, Minecraft Sever, Forums, NamelessMC{/if}">
		...
		<meta property="og:image" content="{if isset($OPAL_META_IMAGE)}{$OPAL_META_IMAGE}{else}https://i.imgur.com/e6YHD.gif{/if}">
		<meta property="og:description" content="{if isset($OPAL_META_HEADING)}{$OPAL_META_DESCRIPTION}{else}We are an online platform to help users around the world connect through NamelessMC powered forums.{/if}">
